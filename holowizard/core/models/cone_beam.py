...
class ConeBeam(BeamSetup):
    ...
    def z12(setup: BeamSetup, measurement: Measurement):
        return setup.z02 * BeamSetup.unit_z02()[1] - measurement.z01 * Measurement.unit_z01()[1]

    @staticmethod
    def get_effective_geometry(setup: BeamSetup, measurement: Measurement):
        z12 = ConeBeam.z12(setup, measurement)
        M = (z12 + measurement.z01 * Measurement.unit_z01()[1]) / (measurement.z01 * Measurement.unit_z01()[1])
    ...
    def get_fr(setup: BeamSetup, measurement: Measurement, verbose=True):
        lam, M, dx_eff, z_eff, fr_eff = ConeBeam.get_effective_geometry(setup, measurement)
        ...
        if verbose is True:
            ...
            logging.info(f"{'Effective dx':<17}{round(dx_eff, 3)} " + BeamSetup.unit_px_size()[0])
            logging.info(f"{'Effective z12':<17}{round(z_eff, 3)} " + Measurement.unit_z01()[0])
