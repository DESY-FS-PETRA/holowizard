...
class FresnelPropagator:
    def __init__(self, fresnel_numbers: List[float], data_shape, running_device):
        ...
        self.fresnel_kernels_conj = [
            kernel_func_conj(distance).to(running_device) for distance in range(self.num_distances)
        ]

        logging.info("Initializing Fresnel propagation model with fresnel numbers:")
        [logging.info("Fr: " + str(self.fresnel_numbers[distance])) for distance in range(self.num_distances)]
    ...
    def propagate_back(self, x, distance):
        propagated = torch.fft.ifft2(torch.fft.fft2(x) * self.fresnel_kernels_conj[distance])
        return propagated
